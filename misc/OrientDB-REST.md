...
OrientDB RESTful HTTP protocol allows to talk with a [OrientDB Server instance](../internals/DB-Server.md) using the [HTTP protocol](http://en.wikipedia.org/wiki/Hypertext_Transfer_Protocol) and [JSON](http://en.wikipedia.org/wiki/JSON). OrientDB supports also a highly optimized [Binary protocol](../internals/Network-Binary-Protocol.md) for superior performances.
...
| **[document](OrientDB-REST.md#document)**<br>Operations on documents by RID<br>[GET](#get---document) - [HEAD](#head---document) - [POST](OrientDB-REST.md#post---document) - [PUT](OrientDB-REST.md#put---document) - [DELETE](OrientDB-REST.md#delete---document) - [PATCH](OrientDB-REST.md#patch---document)| **[documentbyclass](#document-by-class)**<br>Operations on documents by Class | **[export](#export)**<br>Exports a database | **[function](#function)**<br>Executes a server-side function
| **[index](#index)**<br>Operations on indexes | **[listDatabases](#list-databases)**<br>Available databases | **[property](#property)**<br>Operations on schema properties | **[query](#query)**<br>Query |
|**[server](#server)**<br>Information about the server
...
Creation of functions, when not using the Java API, can be done through the Studio in either Orient DB SQL or Java – see the [OrientDB Functions page](../admin/Functions.md).
