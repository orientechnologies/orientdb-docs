...
Users (that are stored in the database in the class _Profile_) can freely register to the social platform and start making friends (friendship is expressed via the _HasFriend_ edge).

Some of the users come become Customers. When this happens the application in use at the Social Travel Agency creats a vertex in the _Customers_ class and links it to the associated Profile via an _HasProfile_ edge.

When Customers are created, they are automatically linked to a Country as well, via an _IsFromCountry _ edge. 
...
When customer start visiting Attractions (like Castles, Momuments, Theatres or Archaeological Sites) or using Services (like Hotels or Restaurants) edges are created to link that specific customer with that specific attractions or service (_HasVisited_, _HasStayed_, and _HasEaten_ edges are used).
