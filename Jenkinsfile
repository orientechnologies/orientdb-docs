    ...
    stage('build')   {
	   ...
	   try{
	      ...
	      withCredentials([usernamePassword(credentialsId: 'orientdb_website', passwordVariable: 'RSYNC_PASSWORD', usernameVariable: 'RSYNC_USERNAME')]) {
                  sh '''
               	      rsync -ratlz --rsh="/usr/bin/sshpass -p ${RSYNC_PASSWORD} ssh -o StrictHostKeyChecking=no -l ${RSYNC_USERNAME}" orientdb-docs/_book/ orientdb.com:/home/orientdb/public_html/docs/3.1.x 
                     '''
              }
