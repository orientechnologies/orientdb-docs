...
- **`FROM`** Designates the object to query.  This can be a class, cluster, single [Record ID](../datamodeling/Concepts.md#recordid), set of [Record ID's](../datamodeling/Concepts.md#recordid), or (beginning in version 1.7.7) index values sorted by ascending or descending key order.
