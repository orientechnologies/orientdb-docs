...
To retrieve a persistent class use the <code>getClass(String)</code> method. If the class not exists NULL is returned.
```java
...
To work in schema-full mode set the strict mode at class level by calling the <code>setStrictMode(true)</code> method. In this case record of that class can't have not-defined properties.
...
- **Minimum value**, accepts a string because works also for date ranges <code>setMin()</code>
- **Maximum value**, accepts a string because works also for date ranges <code>setMax()</code>
- **Mandatory**, it must be specified <code>setMandatory()</code>
- **Readonly**, it may not be updated after record is created <code>setReadonly()</code>
- **Not Null**, can't be NULL <code>setNotNull()</code>
