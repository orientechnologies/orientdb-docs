...
|field[&lt;indexes&gt;](&lt;item&gt;].md)|Document field part. To know more about field part look at the full syntax: [Document_Field_Part](Document-Field-Part.md)|where tags[name='Hi'] or tags[0-3] IN ('Hello') and employees IS NOT NULL|1.0rc5|
...
|--------|---------------|-----------|-----|
|$parent|Get the parent context from a sub-query. Example: select from V let $type = ( traverse * from $parent.$current.children )|[SELECT](SQLSelect.md) and [TRAVERSE](SQL-Traverse.md)|1.2.0|
|$current|Current record to use in sub-queries to refer from the parent's variable|[SELECT](SQLSelect.md) and [TRAVERSE](SQL-Traverse.md)|1.2.0|
